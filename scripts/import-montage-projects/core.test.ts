...
import {
  ...
  collectDistinctEditors, disambiguateFingerprints, type MontageImportRow,
} from './core'
...
describe('disambiguateFingerprints — реальные строки с одинаковым fingerprint', () => {
  function makeRow(sheetRow: number, fingerprint: string): MontageImportRow {
    return {
      ...
      clientMatch: { clientId: 'c1', clientName: 'Клиент', kind: 'exact' }, needsClientReview: false,
      executorPrimaryRaw: '', executorExtraRaw: [], executorKey: null,
      fingerprint, action: 'create',
    }
  }

  it('leaves unique fingerprints untouched', () => {
    const rows = [makeRow(2, 'aaa'), makeRow(3, 'bbb')]
    disambiguateFingerprints(rows)
    expect(rows.map(r => r.fingerprint)).toEqual(['aaa', 'bbb'])
  })

  it('suffixes colliding fingerprints in sheetRow order, leaving the first occurrence untouched', () => {
    // Реальный случай: несколько строк "СТЭП" в один день с одинаковым
    // названием/суммой/исполнителем — разные проекты, но buildFingerprint
    // считает их одинаковыми по заданным ТЗ полям.
    const rows = [makeRow(51, 'stap'), makeRow(52, 'stap'), makeRow(53, 'stap')]
    disambiguateFingerprints(rows)
    expect(rows.map(r => r.fingerprint)).toEqual(['stap', 'stap#2', 'stap#3'])
  })

  it('is stable across repeated runs given the same row order (idempotency)', () => {
    const run1 = [makeRow(51, 'stap'), makeRow(52, 'stap')]
    const run2 = [makeRow(51, 'stap'), makeRow(52, 'stap')]
    disambiguateFingerprints(run1)
    disambiguateFingerprints(run2)
    expect(run1.map(r => r.fingerprint)).toEqual(run2.map(r => r.fingerprint))
  })
})
...
describe('collectDistinctEditors — нормализация + выбор отображаемого варианта', () => {
  ...
  it('ignores rows that are not going to be created (already imported)', () => {
    const rows = [makeRow({ action: 'skip_already_imported' })]
    expect(collectDistinctEditors(rows)).toHaveLength(0)
  })

  it('still counts rows that will be created without a confident client match', () => {
    const rows = [makeRow({ needsClientReview: true, clientMatch: { clientId: null, clientName: null, kind: 'none' } })]
    expect(collectDistinctEditors(rows)).toHaveLength(1)
  })

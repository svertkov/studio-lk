...
// Источник истины — длительность в минутах (Order.makeupDurationMinutes),
// НЕ отдельно хранимое время начала: интервал гримёра всегда пересчитывается
// от plannedStartTime основной съёмки (см. computeMakeupInterval), поэтому
// перенос съёмки не может рассинхронизировать сохранённое "время гримёра".
// ============================================================
...
// заказа, не ограничивают ручной ввод.
...
// ============================================================
// БЫСТРЫЕ ШАБЛОНЫ КОММЕНТАРИЯ — конфигурация массивом, а не отдельными
// кнопками в JSX, чтобы новые шаблоны добавлялись без переписывания
// компонента (см. ТЗ, часть 3). Пока один шаблон — про акцию на первую запись.
// ============================================================
...
export function hasQuickCommentTemplate(comment: string, templateText: string): boolean {
  return comment.includes(templateText)
}

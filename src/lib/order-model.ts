...
// Плотный ("dense") режим — ортогонален tier: внутри 'full' (8 колонок,
// 1920–1280px измеренная ширина контейнера в реальном приложении, где
// боковое меню всегда 240px и не сворачивается) отдельно решает, достаточно
// ли места для полных подписей (акция целиком, кнопка "Комментарий" с
// текстом) или нужны укороченные варианты (см. ТЗ систем­ной пересборки
// таблицы: "не через обрезание многоточием — через заранее заданный
// короткий вариант"). Порог подобран и проверен вживую через Playwright:
// на 1440px измеренная ширина контейнера ≈1134px (не dense, полные подписи),
// на 1366px ≈1060px (dense, короткие подписи) — граница между ними.
export const ORDERS_TABLE_DENSE_MAX_WIDTH = 1100

export function isOrdersTableDense(containerWidth: number): boolean {
  return containerWidth <= ORDERS_TABLE_DENSE_MAX_WIDTH
}
